...
- 共 **30 天**，分 **5 周**进行（最后一周 2 天），每周有截止时间
...
| 第 4 周 | Day 22 ~ Day 28 | 2026-03-30 00:00 (UTC+8) |
| 第 5 周 | Day 29 ~ Day 30 | 2026-04-01 00:00 (UTC+8) |
...
- 检查 PR 标签中是否包含 `weekN-late`（`N` = 1~5）
- 正则匹配：`/week(\d+)[_\-\s]?late/i`
- 周状态展示（已截止 / 进行中 / 未开始）以每周 `deadline` 为基准，
  并使用**上一周 deadline 作为下一周开始边界**，确保最后一周（2 天）判断准确
